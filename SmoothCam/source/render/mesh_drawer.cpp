...
Render::MeshDrawer::MeshDrawer(MeshCreateInfo& info, std::shared_ptr<Render::CBuffer>& perObjectBuffer, D3DContext& ctx) :
...
void Render::MeshDrawer::CreateObjects(std::vector<Model::Vertex>& vertices, D3DContext& ctx) {
	...
	vbo = std::make_unique<Render::VertexBuffer>(vbInfo, ctx);
	context = ctx;
}
...
void Render::MeshDrawer::SetShaders(std::shared_ptr<Shader>& nvs, std::shared_ptr<Shader>& nps) {
	IALayout iaLayout;
